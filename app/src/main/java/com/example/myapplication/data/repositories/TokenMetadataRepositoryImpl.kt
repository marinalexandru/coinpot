...
class TokenMetadataRepositoryImpl @Inject constructor(
    private val tokenMetadataMemoryCache: TokenMetadataMemoryCache
) : TokenMetadataRepository {
    private val tokenDod: DataObservableDelegate<Any, List<TokenMetadata>> = DataObservableDelegate(
        fromNetwork = {
    ...
    override fun observeTokens(forceReload: Boolean): Observable<Data<List<TokenMetadata>>> {
        return tokenDod.observe(Unit, forceReload = true)
    }

    private fun getTokens(): List<TokenMetadata> {
        return listOf(
            TokenMetadata(
                id = 1,
